...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALSoundDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALSoundDetection CreateService(IQiSession session)
		{
			var result = new ALSoundDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALSoundDetection CreateUninitializedService(IQiSession session)
		{
			return new ALSoundDetection(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALSoundDetection");
			}
		}

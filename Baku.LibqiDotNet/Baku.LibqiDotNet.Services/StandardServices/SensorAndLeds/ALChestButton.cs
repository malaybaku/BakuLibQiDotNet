...
namespace Baku.LibqiDotNet.Service
{
    ...
    public class ALChestButton
	{
		internal ALChestButton(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALChestButton CreateService(IQiSession session)
		{
			var result = new ALChestButton(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALChestButton CreateUninitializedService(IQiSession session)
		{
			return new ALChestButton(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALChestButton");
			}
		}
		...
		/// <returns></returns>
        public void _setEnableNotifications(bool arg0_enable)
        {
            SourceService["_setEnableNotifications"].Call(arg0_enable);
        }

        /// <summary>Enables / Disables notifications.</summary>
		/// <param name="arg0_enable">If True enable notifications. If False disable notifications.</param>
		/// <returns></returns>
        public IQiFuture _setEnableNotificationsAsync(bool arg0_enable)
        {
            return SourceService["_setEnableNotifications"].CallAsync(arg0_enable);
        }

        /// <summary>Return true if notifications are active.</summary>
		/// <returns>Return True if notifications are active.</returns>
        public bool _areNotificationsEnabled()
        {
            return SourceService["_areNotificationsEnabled"].Call<bool>();
        }

        /// <summary>Return true if notifications are active.</summary>
		/// <returns>Return True if notifications are active.</returns>
        public IQiFuture<bool> _areNotificationsEnabledAsync()
        {
            return SourceService["_areNotificationsEnabled"].CallAsync<bool>();
        }

...
namespace Baku.LibqiDotNet.Service
{
    ...
    public class ALFsr
	{
		internal ALFsr(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALFsr CreateService(IQiSession session)
		{
			var result = new ALFsr(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALFsr CreateUninitializedService(IQiSession session)
		{
			return new ALFsr(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALFsr");
			}
		}

...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALSensors(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALSensors CreateService(IQiSession session)
		{
			var result = new ALSensors(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALSensors CreateUninitializedService(IQiSession session)
		{
			return new ALSensors(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALSensors");
			}
		}
        ...
        public void Run()
        {
            SourceService["run"].Call();
        }

        /// <summary>Monitors sensors.</summary>
		/// <returns></returns>
        public IQiFuture RunAsync()
        {
            return SourceService["run"].CallAsync();
        }

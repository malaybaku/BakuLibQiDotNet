...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALSonar(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALSonar CreateService(IQiSession session)
		{
			var result = new ALSonar(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALSonar CreateUninitializedService(IQiSession session)
		{
			return new ALSonar(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALSonar");
			}
		}
		...
		/// <returns></returns>
        public IQiResult GetFilteredValues()
        {
            return SourceService["getFilteredValues"].Call<IQiResult>();
        }

        /// <summary>Returns a vector with rightand left filtered sonar values (in this order). Theses valuesare the distances to the nearest detected obstacle.</summary>
		/// <returns></returns>
        public IQiFuture<IQiResult> GetFilteredValuesAsync()
        {
            return SourceService["getFilteredValues"].CallAsync<IQiResult>();
        }

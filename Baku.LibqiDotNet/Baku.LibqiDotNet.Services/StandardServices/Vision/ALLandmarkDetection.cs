...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALLandMarkDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALLandMarkDetection CreateService(IQiSession session)
		{
			var result = new ALLandMarkDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALLandMarkDetection CreateUninitializedService(IQiSession session)
		{
			return new ALLandMarkDetection(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALLandMarkDetection");
			}
		}

...
namespace Baku.LibqiDotNet.Service
{
    ...
    public class ALBarcodeReader
	{
		internal ALBarcodeReader(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALBarcodeReader CreateService(IQiSession session)
		{
			var result = new ALBarcodeReader(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALBarcodeReader CreateUninitializedService(IQiSession session)
		{
			return new ALBarcodeReader(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALBarcodeReader");
			}
		}

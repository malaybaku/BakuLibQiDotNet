...
namespace Baku.LibqiDotNet.Service
{
    ...
    public class ALBacklightingDetection
	{
		internal ALBacklightingDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALBacklightingDetection CreateService(IQiSession session)
		{
			var result = new ALBacklightingDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALBacklightingDetection CreateUninitializedService(IQiSession session)
		{
			return new ALBacklightingDetection(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALBacklightingDetection");
			}
		}

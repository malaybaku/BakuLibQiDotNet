...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALRedBallDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALRedBallDetection CreateService(IQiSession session)
		{
			var result = new ALRedBallDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALRedBallDetection CreateUninitializedService(IQiSession session)
		{
			return new ALRedBallDetection(session);
		}

...
{
    /// <summary></summary>
    public class ALCloseObjectDetection
	{
		internal ALCloseObjectDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALCloseObjectDetection CreateService(IQiSession session)
		{
			var result = new ALCloseObjectDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALCloseObjectDetection CreateUninitializedService(IQiSession session)
		{
			return new ALCloseObjectDetection(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALCloseObjectDetection");
			}
		}

...
namespace Baku.LibqiDotNet.Service
{
    ...
    public class ALSittingPeopleDetection
	{
		internal ALSittingPeopleDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALSittingPeopleDetection CreateService(IQiSession session)
		{
			var result = new ALSittingPeopleDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALSittingPeopleDetection CreateUninitializedService(IQiSession session)
		{
			return new ALSittingPeopleDetection(session);
		}
		...
		public void InitializeService()
		{
			if (!IsInitialized)
			{
			    SourceService = Session.GetService("ALSittingPeopleDetection");
			}
		}
		...
		/// <returns></returns>
        public void SetSittingThreshold(float arg0_threshold)
        {
            SourceService["setSittingThreshold"].Call(arg0_threshold);
        }

        /// <summary>Sets sitting threshold</summary>
		/// <param name="arg0_threshold">float, face height (in meter) from the ground under which we consider thatsomeone is sitting.</param>
		/// <returns></returns>
        public IQiFuture SetSittingThresholdAsync(float arg0_threshold)
        {
            return SourceService["setSittingThreshold"].CallAsync(arg0_threshold);
        }

        /// <summary>Gets sitting threshold</summary>
		/// <returns>Maximum face height in meter</returns>
        public float GetSittingThreshold()
        {
            return SourceService["getSittingThreshold"].Call<float>();
        }

        /// <summary>Gets sitting threshold</summary>
		/// <returns>Maximum face height in meter</returns>
        public IQiFuture<float> GetSittingThresholdAsync()
        {
            return SourceService["getSittingThreshold"].CallAsync<float>();
        }

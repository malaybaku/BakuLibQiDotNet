...
namespace Baku.LibqiDotNet.Service
{
	...
	{
		internal ALWavingDetection(IQiSession session)
		{
			Session = session;
		}
		...
		public static ALWavingDetection CreateService(IQiSession session)
		{
			var result = new ALWavingDetection(session);
		...
		///</summary>
        /// <param name="session">サービスの取得元となるセッション</param>
		public static ALWavingDetection CreateUninitializedService(IQiSession session)
		{
			return new ALWavingDetection(session);
		}
		...
		/// <param name="arg0_maxDistance">New maximum distance (in meters), between 0.5m and 3m</param>
		/// <returns></returns>
        public IQiFuture SetMaxDistanceAsync(float arg0_maxDistance)
        {
            return SourceService["setMaxDistance"].CallAsync(arg0_maxDistance);
        }
        ...
        {
            return SourceService["getMaxDistance"].Call<float>();
        }

        /// <summary>Get maximum distance for waving detection</summary>
		/// <returns>Current maximum distance (in meters)</returns>
        public IQiFuture<float> GetMaxDistanceAsync()
        {
            return SourceService["getMaxDistance"].CallAsync<float>();
        }
		...
		/// <param name="arg0_sensitivity">New minimum size (in meters), between 0.05m and 0.3m</param>
		/// <returns></returns>
        public IQiFuture SetMinSizeAsync(float arg0_sensitivity)
        {
            return SourceService["setMinSize"].CallAsync(arg0_sensitivity);
        }
        ...
        {
            return SourceService["getMinSize"].Call<float>();
        }

        /// <summary>Get minimum size of movement for waving detection</summary>
		/// <returns>Current minimum size (in meters)</returns>
        public IQiFuture<float> GetMinSizeAsync()
        {
            return SourceService["getMinSize"].CallAsync<float>();
        }

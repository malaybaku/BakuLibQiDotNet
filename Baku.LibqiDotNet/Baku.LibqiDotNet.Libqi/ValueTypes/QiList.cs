...
namespace Baku.LibqiDotNet.Libqi
{
    ...
    public sealed class QiList<T> : QiInputValue 
        where T : QiInputValue
    {
        ...
        /// <summary>
        /// 列挙された<see cref="QiInputValue"/>派生型から、それに対応したリストを生成します。
        /// </summary>
        ...
        /// <summary>
        /// 列挙された<see cref="QiInputValue"/>派生型を要素として含む、動的型の内容からなるリストを生成します。
        /// </summary>
        /// <param name="values">何かしらの値の列挙</param>
        /// <returns>指定した値を保持する動的型のリスト</returns>
        public static QiList<QiDynamic> CreateDynamic(IEnumerable<QiInputValue> values)
        {
            if (!values.Any())
            {
                throw new InvalidOperationException("values length must be greater than 0");
    ...
    public static class QiList
    {
        ...
        /// <returns>指定した値を保持する動的型のリスト</returns>
        public static QiList<QiDynamic> CreateDynamic(IEnumerable<QiInputValue> values)
            => QiList<QiInputValue>.CreateDynamic(values);
    ...
    /// <summary><see cref="QiList{T}"/>の拡張メソッドを定義します。</summary>
    public static class QiListExtension
    {
        ...
        public static QiList<QiDynamic> ToQiDynamicList<T>(this IEnumerable<T> values)
            where T : QiInputValue
            => QiList.CreateDynamic(values.Cast<QiInputValue>());

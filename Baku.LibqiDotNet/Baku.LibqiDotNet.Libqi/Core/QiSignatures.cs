...
namespace Baku.LibqiDotNet.Libqi
{
    ...
    public static class QiSignatureValidityChecker
    {
        ...
        /// <returns></returns>
        public static bool CheckValidity(string signature, QiInputValue[] args)
        ...
        /// <summary>タプルのシグネチャを要素別に分解します。</summary>
        /// <param name="signatures">タプルのシグネチャ(例: "(ib[s])"</param>
        /// <param name="ignoreError">パースに失敗したシグネチャを自動で<see cref="QiInputValue"/>とみなすかどうかを設定します。</param>
        /// <returns>分解された要素別のシグネチャ(例:入力が"(ib[s])"なら{ "i", "b", "[s]" })</returns>
